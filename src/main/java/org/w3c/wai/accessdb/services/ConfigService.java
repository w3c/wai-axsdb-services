...
public enum ConfigService {
	...
	private void init(){
		this.configDefault.put(FORM_TESTUNIT_FORMFIELD_TESTUNITDESCRIPTION,
				"TestUnitDescription");
		this.configDefault.put(FORM_TESTUNIT_FORMFIELD_CODE,
				"testCode"); 
		...
		try {
			...
			try {
				config.load(new FileInputStream(System.getProperty("user.home")+  "/.accessdb/" + "config.properties"));
				//logger.info("success");
			} catch (IOException e) {
				try {
					//logger.info("Trying to find config.properties in " + ConfigService.class.getResource("/").getPath() + "config.properties");
					config.load(new FileInputStream(ConfigService.class.getResource("/").getPath() + "config.properties"));
					//logger.info("success");
	...
	private void evalVal()
	{
		...
		while (em.hasMoreElements()) {
			...
			String val = this.config.getProperty(str,	this.configDefault.getProperty(str));
