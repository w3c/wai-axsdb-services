...
public class TestUnitsResource
{
            ...
            @PathParam("testUnitId") String testUnitId)
    {
        ...
        try
        {
            ...
            for (RefFileType r : testUnitDescription.getSubject().getResourceFiles())
            {
                if(r.getId() == refFile.getId())
                {
                    testUnitDescription.getSubject().getResourceFiles().remove(r);
                    break;
                }
            }
            EAOManager.INSTANCE.getTestUnitDescriptionEAO().persist(testUnitDescription);
            if (file.delete())
